...
export class CategoryService {
  ...
  newCategory(request): Observable<any> {
    return this.http.post<any>(pathBack + 'categories', JSON.stringify(request), {
      headers: new HttpHeaders()
  ...
  updateCategory(id, request): Observable<any> {
    return this.http.put<any>(`${pathBack}categories/${id}`, JSON.stringify(request), {
      headers: new HttpHeaders()
          .set('Content-Type', 'application/json'),
      observe: 'response'
  }).pipe(
  ...
  deleteCategory(id): Observable<any> {
    return this.http.delete<any>(`${pathBack}categories/${id}`, {
      headers: new HttpHeaders()
          .set('Content-Type', 'application/json'),
      observe: 'response'
  }).pipe(
